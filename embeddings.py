...
def create_tsne_map_with_legend_and_save(embeddings_file, labels, size, prefix):
    ...
    plt.scatter(positiv_points[:, 0], positiv_points[:, 1], color='blue', label='Positive')
    plt.scatter(negativ_points[:, 0], negativ_points[:, 1], color='red', label='Negative')
...
def main():
    ...
    for size, dataset in tqdm(nested_splits_tokenized.items(), desc="Creating t-SNE plots"):
